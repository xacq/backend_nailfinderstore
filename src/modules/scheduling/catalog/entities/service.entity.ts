...
@Entity('services')
@Index('fk_services_business', ['businessId'])
@Index('fk_services_category', ['categoryId'])
export class Service extends SoftDeletableEntity {
  ...
  @Column({ name: 'preparation_time_minutes', type: 'int', default: () => '0' })
  preparationTimeMinutes!: number; // :contentReference[oaicite:14]{index=14}
  @Column({ name: 'buffer_time_minutes', type: 'int', default: () => '0' })
  bufferTimeMinutes!: number; // :contentReference[oaicite:15]{index=15}
  @Column({ name: 'is_active', type: 'tinyint', default: () => '1' })
  isActive!: boolean; // :contentReference[oaicite:16]{index=16}
}
