...
@Index('fk_business_hours_business', ['business'])
export class BusinessHour extends TimestampedEntity {
  @ManyToOne(() => Business, { onDelete: 'CASCADE' })
  business!: Business;
  @Column({ name: 'business_id' }) businessId!: string;                 // :contentReference[oaicite:24]{index=24}

  @ManyToOne(() => BusinessLocation, { nullable: true, onDelete: 'SET NULL' })
  @JoinColumn({ name: 'location_id' })
  location?: BusinessLocation | null;
  @Column({ name: 'location_id', nullable: true }) locationId?: string | null; // :contentReference[oaicite:25]{index=25}
  ...
  @Column({ name: 'is_closed', type: 'tinyint', default: () => 0 }) isClosed!: boolean; // :contentReference[oaicite:29]{index=29}
}
