...
export class ServiceReview extends TimestampedEntity {
  ...
  business!: Business;
  @Column({ name: 'business_id' }) businessId!: string;              // :contentReference[oaicite:51]{index=51}

  @ManyToOne(() => Appointment, { nullable: true, onDelete: 'SET NULL' })
  @JoinColumn({ name: 'appointment_id' })
  appointment?: Appointment | null;
  @Column({ name: 'appointment_id', nullable: true }) appointmentId?: string | null; // :contentReference[oaicite:52]{index=52}

  @ManyToOne(() => Service, { onDelete: 'CASCADE' })
  @JoinColumn({ name: 'service_id' })
  service!: Service;
  @Column({ name: 'service_id' }) serviceId!: string;                // :contentReference[oaicite:53]{index=53}

  @ManyToOne(() => User, { onDelete: 'CASCADE' })
  @JoinColumn({ name: 'reviewer_id' })
  reviewer!: User;
  @Column({ name: 'reviewer_id' }) reviewerId!: string;              // :contentReference[oaicite:54]{index=54}
